...
CREATE TABLE CGROUP_REL    (
    ...
    UNIQUE KEY CGPREL (CGROUP_ID,CID)
...
CREATE TABLE CUSTOMER_RESOURCE_REL (
    ...
    INDEX (CID, RESOURCE_ID),
    FOREIGN KEY (CID) REFERENCES CUSTOMER(CID) ON DELETE CASCADE,
    FOREIGN KEY (RESOURCE_ID) REFERENCES RESOURCE(ID) ON DELETE CASCADE,
	FOREIGN KEY (SHARED_USER) REFERENCES CUSTOMER(CID) ,
    UNIQUE KEY CGPREL (CID,RESOURCE_ID)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
ALTER TABLE CUSTOMER_RESOURCE_REL ADD START_DATE DATE;
ALTER TABLE CUSTOMER_RESOURCE_REL ADD END_DATE DATE;
...
CREATE TABLE PERMISSION_RECORD    (
	...
	CREATE_USER varchar(20) COMMENT '行为创建人(分享人或者后台用户名)',
    INDEX (RESOURCE_NAME)
)ENGINE=InnoDB  DEFAULT CHARSET=utf8;
...
CREATE TABLE NODE    (
	...
	GRADE int(2) NOT NULL COMMENT '资源节点级别。根节点为0', 
	N_INT1 int(5) , 
    VC1 varchar(20) , 
	CHR1 CHAR(1), 
    INDEX (ID,NAME),
