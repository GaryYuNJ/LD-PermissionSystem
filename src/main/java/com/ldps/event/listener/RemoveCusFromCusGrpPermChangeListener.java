...
public class RemoveCusFromCusGrpPermChangeListener implements ApplicationListener<RemoveCusFromCusGrpPermChangeEvent> {
	...
	public void onApplicationEvent(RemoveCusFromCusGrpPermChangeEvent event) {
		...
		RemoveCusFromCusGrpPermChangeEventData eData = (RemoveCusFromCusGrpPermChangeEventData)event.getSource();
		...
		permRecordModel.setCgroupId(eData.getCusGrpId());
	
		//先找要检查的资源list
		List<Integer> resourceIds = new ArrayList<Integer>();
		
		//获取用户组对应的资源权限
		List<CusGrpResourceRelModel> cGrpResRelModels = iCusGrpResourceRelService.queryByCusGroupId(eData.getCusGrpId());
		...
		//获取用户组对应的资源组权限
		//用户对资源组的关系
		List<CusGroupResGroupRelModel> cusGrpResGrpRelModels = cusGroupResGroupRelDao.selectByCusGroupId(eData.getCusGrpId());
		...
		for(Integer resId : resourceIds){
			...
			if(null == permRecordList || permRecordList.size() > 0){
				...
				//如果是
				if(null != permRecordList.get(0).getCgroupId() && eData.getCusGrpId() == permRecordList.get(0).getCgroupId()){
					...
					if(permRecord.getActionType() == 0){
						...
						iCusResourceRelService.disableResourcePermission(relModel);
						...
						permissionRecordDao.insertSelective(permRecord);
					}else{
						//恢复上次的授权状态
			...
			//没有权限记录，直接删除关系（连带保存取消授权记录）
				...
				permRecord.setCreateUser(0L);
				permRecord.setCreateDate(new Date());
				permRecord.setCgroupId(eData.getCusGrpId());
				//insert
				permissionRecordDao.insertSelective(permRecordModel);
			}
		}
	}
}
